const endpoints = [
  ...
  ["POST", "/api/v1/agents", "创建链上钱包地址 + API Key + wallet_state"],
  ["PUT", "/api/v1/agents", "Faucet topup USDC / 更新 policy"],
  ["GET", "/api/v1/resources", "列出可购买资源 + pay_to 地址 + resource_token"],
  ["GET", "/api/v1/resources/:id", "无支付返回 HTTP 402 + x402 accepts"],
  ["POST", "/api/v1/pay", "链上/local USDC 自动结算（真实 tx_hash）"],
  ["POST", "/api/v1/agent/cycle", "一键全自动：开户→上架→支付→收款"],
  ["GET", "/api/v1/chain/tx/:hash", "校验结算交易"],
  ["POST", "/api/v1/swap/quote", "Failover pool 兑换询价"],
  ["GET", "/api/v1/ledger", "审计导出 / 卖方核对入账"],
  ["GET", "/api/v1/openapi", "机器可读能力发现（含 x-settlement）"],
