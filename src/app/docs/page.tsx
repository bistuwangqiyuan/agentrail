const endpoints = [
  ["POST", "/api/v1/principals", "创建 principal（KYA 锚点）"],
  ["POST", "/api/v1/agents", "创建 Agent 钱包 + API Key（无登录表单）"],
  ["GET", "/api/v1/resources", "列出可购买资源"],
  ["GET", "/api/v1/resources/:id", "无支付返回 HTTP 402 + x402 accepts"],
  ["POST", "/api/v1/pay", "Agent 自动支付并取回 payload"],
  ["POST", "/api/v1/swap/quote", "Failover 兑换询价"],
  ["GET", "/api/v1/ledger", "审计导出"],
